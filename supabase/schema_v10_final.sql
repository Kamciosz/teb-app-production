-- ============================================================================
-- SCHEMA_V10 - Final Current State
-- Generated from: schema_v9_clean_modular.sql + all migrations (20260327-20260330)
-- Date: 2026-03-30
-- Purpose: Single current-state schema file for easy deployment/reference
-- ============================================================================
...
drop table if exists public.rewear_messages cascade;
drop table if exists public.rewear_conversations cascade;
drop table if exists public.rewear_interests cascade;
...
drop function if exists public.can_access_rewear_conversation(uuid) cascade;
drop function if exists public.start_rewear_conversation(bigint, text) cascade;
drop function if exists public.touch_rewear_conversation_last_message() cascade;
drop function if exists public.complete_rewear_purchase(uuid) cascade;
...
create table public.rewear_posts (
  ...
  title text not null,
  description text,
  price_teb_gabki integer not null default 0,
  ...
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  constraint rewear_posts_title_length_check check (char_length(title) <= 200),
  constraint rewear_posts_description_length_check check ((description is null) or (char_length(description) <= 5000)),
...
create table public.rewear_interests (
...
create table public.rewear_conversations (
  id uuid primary key default gen_random_uuid(),
  post_id bigint not null references public.rewear_posts(id) on delete cascade,
  seller_id uuid not null references public.profiles(id) on delete cascade,
...
create table public.rewear_messages (
  id uuid primary key default gen_random_uuid(),
  ...
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  constraint rewear_messages_content_length_check check (char_length(content) <= 2000)
);
...
create index rewear_interests_user_created_idx on public.rewear_interests(user_id, created_at desc);
create index rewear_interests_post_created_idx on public.rewear_interests(post_id, created_at desc);
create index rewear_conversations_seller_last_message_idx on public.rewear_conversations(seller_id, last_message_at desc);
create index rewear_conversations_buyer_last_message_idx on public.rewear_conversations(buyer_id, last_message_at desc);
create index rewear_conversations_post_idx on public.rewear_conversations(post_id, created_at desc);
create index rewear_messages_conversation_created_idx on public.rewear_messages(conversation_id, created_at asc);
create index rewear_messages_sender_created_idx on public.rewear_messages(sender_id, created_at desc);
...
for each row execute function public.touch_rewear_conversation_last_message();
  ...
  select exists (
    ...
    where rc.id = conv_id
      and (
        ...
        or auth.uid() = rc.seller_id
        or auth.uid() = rc.buyer_id
      )
  );
$$;
...
for select
to authenticated
using (public.can_access_rewear_conversation(id));

create policy rewear_messages_select_participants
on public.rewear_messages
for select
to authenticated
using (public.can_access_rewear_conversation(conversation_id));

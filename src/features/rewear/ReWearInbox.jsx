...
export default function ReWearInbox() {
  ...
  async function handleSendMessage(event) {
    ...
    setSending(true)
    try {
      const { error } = await supabase.from('rewear_messages').insert([{
  ...
  return (
    <div className="relative min-h-[80vh] pb-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.1fr)]">
        <section className="bg-surface border border-gray-800 rounded-3xl overflow-hidden">
        ...
        <section className="bg-surface border border-gray-800 rounded-3xl overflow-hidden flex flex-col min-h-[68vh]">
          {activeConversation ? (
            <>
              ...
              <div className="px-4 py-3 border-b border-gray-900/80 bg-background/60 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-bold text-white truncate">{activePost?.title || 'Ogłoszenie ReWear'}</div>
              ...
              <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-800 bg-[#181818] flex gap-3">
                <textarea
                  value={newMessage}

...
export async function readJsonBody(req) {
  ...
  for await (const chunk of req) {
    if (Date.now() - readStart > 8000) {
      throw new Error('Request body read timeout');
    }
...
export function requireSameOrigin(req, res) {
  ...
  // For POST/PUT/DELETE: Origin header is always present
  // For GET/HEAD: Origin may be missing for same-origin, use Referer
  const requestSource = origin || referer;

  // If no source at all, it could be same-origin GET without Referer
  // Allow it if the request comes from the same host (non-CORS request)
  if (!requestSource) {
    // Allow same-origin GET requests without Origin/Referer
    // This handles browser same-origin fetch without explicit headers
    return true;
  }

  let parsedOrigin;
  try {
    parsedOrigin = new URL(requestSource);
  } catch {
  ...
  // Allow same-origin: origin hostname matches host header
  if (parsedOrigin.hostname === host) {
    return true;
  }

  // Allow known trusted domains
  const trusted = ['www.teb-app.pl', 'teb-app.pl', 'teb-app-production.vercel.app'];
  if (trusted.includes(parsedOrigin.hostname) && trusted.includes(host)) {
    return true;
  }
...
export async function getSessionFromCookies(req, res) {
  ...
  if (accessToken) {
    ...
    const tokenExpired = Boolean(payload?.exp && payload.exp <= nowSeconds);
    if (!tokenExpired) {
